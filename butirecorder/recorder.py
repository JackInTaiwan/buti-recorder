...
class Recorder :
    ...
    def save_state_dict(self) :
        if len(self.models) < 1 :
            raise ValueError("Method save_state_dict requires Recorder.model_names \
            length > 0, but got {}.".format(len(self.models)))

        else :
            if not isinstance(self.models, dict) and len(self.model_names) != 1 :
                raise ValueError("Method save_state_dict requires that type of  \
                Recorder.models is dict, or length of Recorder.model_names is 1.")
